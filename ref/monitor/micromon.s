...
;
;***************************************************
;*   Micromon 1.2 (c)2013-2019 by Kevin E. Maier   *
;*  Extendable Mini BIOS and Monitor for 65C02 CPU *
;*                                                 *
;* Basic functions include:                        *
;*  - Byte/Text memory search                      *
;*  - CPU register display/modify                  *
;*  - Memory fill, move, compare, display, execute *
;*  - Macro init/run                               *
;*                                                 *
;*                  02/26/2019 KM                  *
;*  Uses 1.75KB EEPROM - JMP table page at $FF00   *
;*     Default I/O page is 256 bytes at $FE00      *
;*        Default assembly start at $F800          *
;*                                                 *
;*  C02BIOS 1.2L (c)2013-2019 by Kevin E. Maier    *
;*  - BIOS in pages $FD, $FF                       *
;*  - Full duplex interrupt-driven/buffered I/O    *
;*  - extendable BIOS structure with soft vectors  *
;*  - soft config parameters for 65C51 UART        *
;*  - monitor cold/warm start soft vectored        *
;*  - Panic routine via NMI trigger                *
;*  - fully relocatable code (sans page $FF)       *
;*                                                 * 
;*   Note default HW system memory map as:         *
;*         RAM - $0000 - $7FFF                     *
;*         ROM - $8000 - $FFFF (less I/O page)     *
;*         I/O - $FE00 - $FEFF                     *
;*                                                 *
;***************************************************
;
...
;*************************
;Reserved from $00 to $CF for user routines
;
PGZERO_ST       .EQU    $D0             ;Start of Page Zero usage
;
;Buffers used by the default Monitor code
; - Two buffers are required; SRCHBUFF is used by the Search Engine - 16 bytes.
; - INBUFF is required for RDLINE - 4 bytes used by HEXINPUT.
;
BUFF_PG0        .EQU    PGZERO_ST       ;Default Page zero location for Monitor buffers
;
INBUFF          .EQU    BUFF_PG0+0      ;4 bytes ($D0-$D3)
;
;16-bit variables:
...
TEMP1L          .EQU    PGZERO_ST+12    ;Index for word temp value used by Memdump
TEMP1H          .EQU    PGZERO_ST+13
TEMP2L          .EQU    PGZERO_ST+14    ;Index for Text entry
TEMP2H          .EQU    PGZERO_ST+15
PROMPTL         .EQU    PGZERO_ST+16    ;Prompt string address
PROMPTH         .EQU    PGZERO_ST+17
SRCL            .EQU    PGZERO_ST+18    ;Source address for memory operations
SRCH            .EQU    PGZERO_ST+19
TGTL            .EQU    PGZERO_ST+20    ;Target address for memory operations
TGTH            .EQU    PGZERO_ST+21
LENL            .EQU    PGZERO_ST+22    ;Length address for memory operations
LENH            .EQU    PGZERO_ST+23
;
;8-bit variables and constants:
BUFIDX          .EQU    PGZERO_ST+24    ;Buffer index
BUFLEN          .EQU    PGZERO_ST+25    ;Buffer length
IDX             .EQU    PGZERO_ST+26    ;Temp Indexing
IDY             .EQU    PGZERO_ST+27    ;Temp Indexing
TEMP1           .EQU    PGZERO_ST+28    ;Temp value - Code Conversion routines
TEMP2           .EQU    PGZERO_ST+29    ;Temp value - Memory/EEPROM/SREC routines
CMDFLAG         .EQU    PGZERO_ST+30    ;Command Flag - used by RDLINE
;
...
;**************************************************************************************************
                .ORG $F800              ;Target address range $F000 through $FDFF will be used
;**************************************************************************************************
...
BSOUT2          LDA     #$08            ;Get another ASCII backspace
                BRA     ASCOUT          ;Branch and send, then return
;
...
RDCHAR          JSR     CHRIN           ;Request keystroke input from terminal
                ...
                BCC     AOK             ;Branch if value < $61, control code, upper-case or numeric
...
GO              SMB7    CMDFLAG         ;Set Command flag
                JSR     SETUP           ;Get HEX address input (Areg/Yreg contains 16-bit value)
...
SENGBR1         JSR     SINCPTR         ;Increment address pointer, test for end of memory
                ...
                SEC                     ;Subtract buffer index from memory addr pointer: Set carry
...
RESET           LDA     #$16            ;Get msg "Reset System"
...
MSG_16          .DB     "RESET"
                .DB     $00
;
MSG_TABLE       ;Message table - contains address words of each message sent via the PROMPT routine
                .DW     MSG_00
                .DW     MSG_01
                .DW     MSG_02
                .DW     MSG_03
...
;START OF BIOS CODE
;**************************************************************************************************
;C02BIOS version used here is 1.2L (6551 only)
;
...
INTERUPT0       LDA     SIOSTAT         ;Get status register, xfer irq bit to n flag
                BPL     REGEXT          ;if clear no 6551 irq, exit, else
...
REGEXT          JMP     (IRQRTVEC0)     ;handle next irq
;
...
DATA_XFLP
                LDA     VEC_TABLE-1,X   ;Get ROM table data
...
;
                ...
                JSR     INIT_6551       ;Init I/O - Console
                BRA     WMBV            ;Jump to Monitor Warm Start Vector
;
;**************************************************************************************************
...
; the default size for the config table is 16 bytes, 14 bytes are free
;
