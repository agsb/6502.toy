;**************************************************************************************************
;*   C02Monitor 4.01 - Release version for Pocket SBC  (c)2013-2022 by Kevin E. Maier 27/01/2022  *
;*                                                                                                *
...
;*      - RTC function based on 10ms Jiffy Clock: Ticks, Seconds, Minutes, Hours, Date            *
...
;*      - CTRL-B: Starts Rich Leary's DOS/65 ROM Version 3                                        *
...
;*      - CTRL-T: Date and Time - values loaded during boot from RTC                              *
...
; $FF48 is Reserved for future expansion (1 available)
;
...
; $FF18 IDE_EN_CACHE    ;Enable the Write Cache on IDE controller
; $FF1B IDE_DIS_CACHE   ;Disable the Write Cache on IDE controller
;
; $FF1E RTC_NVRD        ;Read the NVRAM from the RTC
; $FF21 RTC_NVWR        ;Write the NVRAM to the RTC
; $FF24 RTC_INIT        ;Initialize the Software RTC from the hardware RTC
;
; $FF27 CHRIN2          ;Data input from serial port (Port B)
; $FF2A CHROUT2         ;Data output to serial port (Port B)
;
...
;
; $FF48 Reserved        ;Reserved BIOS call for future expansion
;
; $FF4B INIT_VEC        ;Initialize soft vectors at $0300 from ROM
; $FF4E INIT_CFG        ;Initialize soft config values at $0320 from ROM
; $FF51 INIT_28L92      ;Initialize SC28L92 console 115.2K, 8-N-1 RTS/CTS
...
;**************************************************************************************************
COLD_MON        LDA     #$15            ;Get version msg
                JSR     PROMPT          ;Send to Console
                LDA     #$14            ;Get intro msg / BEEP
                JSR     PROMPT          ;Send to Console
;
;**************************************************************************************************
;*                              Command input loop                                                *
...
; xxxxx.xx seconds.
BENCH           LDA     #$29            ;Get message for Bench startup
...
QUITB           JSR     B_CNT_STOP      ;Stop Benchmark counter
                ...
                LDA     #$48            ;Get message for " Seconds"
                JSR     PROMPT          ;Send to console
...
;
LP_CNT_FL       LDA     #$20            ;Get Loop Count msg
                JSR     PROMPT          ;Send to console
...
NOLOOPS         STZ     ICNT_A          ;Zero Input buffer count
                STZ     ITAIL_A         ;Zero Input buffer tail pointer
MACINI          STZ     IHEAD_A         ;Zero Input buffer head pointer
...
RUNMACRO        LDA     #$7F            ;Get keystroke buffer max length-1
                STA     ITAIL_A         ;Push tail pointer to end
                INC     A               ;Increment to $80 for buffer count (full)
                STA     ICNT_A          ;Make count show as full ($80)
...
PROG_EE         LDA     #$1C            ;Get warning msg
                JSR     PROMPT          ;Send to console
...
PROG_EEP        SMB7    TEMP2           ;Set EEPROM write active mask
                ...
                LDA     #$1F            ;Get Prog failed message
                BRA     BRA_PRMPT       ;Branch to Prompt routine
;
PRG_GOOD        LDA     #$1E            ;Get completed message
...
NO_RAM          LDA     #$2B            ;Get address range message
                JSR     PROMPT          ;Send to console
                LDA     #$2D            ;Get RAM message
                JSR     PROMPT          ;Send to console
...
ASSEMBLER       LDA     #$2E            ;Get Intro message
                JSR     PROMPT          ;Send to console
...
;
                ...
                LDA     #$1F            ;Else, get error msg for failed write
                BRA     PRMPTSC         ;Send to console and exit
;
NOT_ROM         LDA     #$2B            ;Get address range msg
                JSR     PROMPT          ;Send to console
                LDA     #$2C            ;Get EEPROM msg
                BRA     TRY_AGAIN       ;Branch back and try again
;
;[CNTRL-Q] Query command:
QUERY           LDA     #$49            ;Get Query Commands Message
                BRA     PRMPTSC         ;Send to console and return
;
;[CNTL-T] DATE/TIME command: Sends a string to the console showing the current Date and Time
; Displays software RTC values for Date and Time as:
;  Date: SUN Sep 15, 2020 Time: 12:34:56
;
TIME            LDA     #$30            ;Get "Date: " message
                JSR     PROMPT          ;Send to console
;
...
XDONE           LDA     #ACK            ;Last block, get ACK character
                ...
                LDA     #$25            ;Get Good xfer message number
...
STRT_ERR        LDA     #CAN            ;Else get Cancel code
                ...
                LDA     #$26            ;Get message for receive error
...
XMODEMS         STZ     OPXMDM          ;Clear Xmodem flag
                ...
                STA     BLKNO           ;Save it
;
                LDA     #$23            ;Get Xmodem Save intro msg
...
; Block count has been set to one and the OPXMDM flag has been cleared.
                LDA     #$24            ;Get Terminal Setup message
                JSR     PROMPT          ;Send to console
;
...
;
                LDA     #$25            ;Else, get complete msg
XMDM_NE         JMP     PROMPT          ;Send to console and return
;
XMDM_ERR        LDA     #$26            ;Get xfer fail msg
...
;
SYS_RST         LDA     #$1A            ;Get msg "Reset System"
...
ZERO            LDA     #$19            ;Get msg "Zero RAM/Reset System"
...
MONTAB  .DW     ASSEMBLER       ;[CNTRL-A] $01 Start Assembler
        .DW     BOOT_DOS65      ;[CNTRL-B] $02 Boot DOS/65 Rom Version
...
MSG_1A  .DB     "Reset,"
        .DB     $00
MSG_1B  .DB     "Program EEPROM",$0D,$0A
        .DB     $00
MSG_1C  .DB     $0D,$0A
        .DB     "Are you sure? "
        .DB     $00
...
MSG_21  .DB     $0D,$0A
        .DB     "Loops: "
        .DB     $00
MSG_22  .DB     "Xmodem Download, <ESC> to abort, or"
        .DB     $0D,$0A
        .DB     "Load Address/S-Record offset:"
        .DB     $00
MSG_23  .DB     "Xmodem Upload, <ESC> to abort, or"
        .DB     $0D,$0A
...
MSG_28  .DB     $0D,$0A
        .DB     "Disassembly from"
        .DB     $00
MSG_29  .DB     "Start Benchmark counter? "
        .DB     $00
MSG_2A  .DB     "Caution! Editing EEPROM data."
        .DB     $00
MSG_2B  .DB     $0D,$0A
        .DB     "Address not in "
        .DB     $00
